...
export const WorkoutProvider: React.FC<WorkoutProviderProps> = ({ children }) => {
  ...
  useWorkoutPersistence({
    ...
    setExerciseDuration: (duration) => setConfig(prev => ({ ...prev, exerciseDuration: duration })),
    setRestDuration: (duration) => setConfig(prev => ({ ...prev, restDuration: duration })),
  });

  // Derived values
  const validExercises = useMemo(() => config.exercises.filter(e => e.trim()), [config.exercises]);
  
  const currentExercise = useMemo(() => {
    if (!session || validExercises.length === 0) return null;
    const exercise = validExercises[session.currentExerciseIndex % validExercises.length];
    return exercise;
  }, [session, validExercises]);

  const nextExercise = useMemo(() => {
    if (!session || validExercises.length === 0) return null;
    return validExercises[(session.currentExerciseIndex + 1) % validExercises.length];
  }, [session, validExercises]);
  ...
  const setProgress = useMemo(() => {
    if (!session || screen === 'config' || screen === 'betweenSets' || screen === 'complete') {
      return 0;
    }
  ...
  const reorderExercises = useCallback((dragIndex: number, dropIndex: number) => {
    setConfig(prev => {
      const newExercises = [...prev.exercises];
  ...
  const completeExercise = useCallback(() => {
    ...
    const isLastExercise = session.currentExerciseIndex === validExercises.length - 1;
    ...
    if (isLastExercise) {
      ...
      console.log('😴 Going to rest, next up:', validExercises[session.currentExerciseIndex + 1]);
      setSession(prev => prev ? { ...prev, currentSegmentIndex: prev.currentSegmentIndex + 1 } : null);
      setScreen('rest');
    }
  }, [session, validExercises]);
